...
public class Main extends Applet {
    ...
    public static void trainMore() {
        ...
        keepSoloRecord.description = "运行10次，每次10000000次对练，德鲁伊10命中加成，刺客瞬杀10miss";
        keepSoloRecord.save("./better");
    }

    private static SoloRecord train(int[] ends, final SoloRecord record) {
        Game soloGame = new SoloGame(null);
        Player p1 = new Druid(new SoloBot(record));
        Player p2 = new Assassin(new SoloBot(record));
    ...
    private static void play() {
        ...
        soloGame.print = true;
        SoloRecord record = new SoloRecord("./better");
        Player p1 = new Druid();
        Player p2 = new Assassin(new SoloBot(record));
        soloGame.addPlayer(p1, 1);
        soloGame.addPlayer(p2, 2);
